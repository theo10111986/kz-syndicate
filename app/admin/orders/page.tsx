...
export default async function AdminOrders({ searchParams }: { searchParams?: SearchParams }) {
  ...
  return (
    <main className="kz-wrap">
      <KZStyles />
      <h2 className="kz-subtitle">Orders</h2>

      {/* Φίλτρα */}
      <form className="kz-filters" action="/admin/orders" method="GET">
        <input name="q" defaultValue={q} placeholder="Αναζήτηση (email, order id)" />
          ...
          <tbody>
            {pageRows.map((o:any) => (
              <tr key={o.id}>
                ...
                <td>{o.user?.email || "-"}</td>
                <td>{Number(o.total)} {o.currency}</td>
              </tr>
            ))}
            {!pageRows.length && (
      ...
      <div className="kz-pager">
        {page > 1 && (
          <Link className="kz-link" href={`/admin/orders?page=${page-1}${q?`&q=${encodeURIComponent(q)}`:""}${searchParams?.from?`&from=${searchParams.from}`:""}${searchParams?.to?`&to=${searchParams.to}`:""}`}>← Προηγούμενα</Link>
        )}
        {hasNext && (
          <Link className="kz-link" href={`/admin/orders?page=${page+1}${q?`&q=${encodeURIComponent(q)}`:""}${searchParams?.from?`&from=${searchParams.from}`:""}${searchParams?.to?`&to=${searchParams.to}`:""}`}>Επόμενα →</Link>
        )}
      </div>
    </main>
  );
